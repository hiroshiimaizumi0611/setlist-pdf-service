...
- 編集画面側で今回触るのは `PDF出力` の入口を preview page 遷移へ差し替える範囲だけに限定する
- 編集画面の追加レイアウト変更や新機能追加は今回の対象外
...
### 2.5. Multi-page preview behavior

preview page の左カラムは「1枚だけの疑似サムネイル」ではなく、layout builder が返した全ページを上から順に縦積み表示する。  
そのため、複数ページに分かれる event では preview も `page 1`, `page 2` ... をそのまま再現する。
...
preview / PDF の一致を保証するため、以下はすべて `layout builder` の責務に寄せる。

- 行の順番
- ページ分割
- cue label (`M01`, `M02`, `--`, `EN` など)
- 種別ごとの row variant
- 各タイトルの表示用テキスト
- 長いタイトルの圧縮 / 折り返し判定
- warning 判定

preview presenter と pdf renderer は独自に pagination や warning 判定をしない。  
両者は builder が返した `page`, `row`, `displayText`, `warning` をそのまま描画するだけにする。
...
Stitch 準拠の判断基準は、以下の reference を source of truth とする。
...
- 対応する PNG preview image

実装時に must-match とする visual primitives:
...
- `MC` の軽い区切り表現
- `転換` の中央寄せ separator
- `見出し` の section-break 行
- 明確に異なる light / dark の紙面コントラスト
...
Stitch preview 内の演出要素のうち、実運用上の意味が薄いものは PDF にも preview sheet にも入れない。
...
- 紙面プレビューを中央寄せ
- preview は PDF と同じ page / row 構造を React で表現
- decorative copy の削除ルールも PDF と同じものを適用する
...
- 初回版は `長い曲名` 警告だけを扱う
- `ページ跨ぎ` や `見出し直後の改ページ` の警告は将来拡張に回す
- 将来的な warning type 追加に耐えられるデータ構造にはしておく
...
1. layout builder がテーマごとの紙面ルールに従って許容幅を計測する
2. builder が圧縮または折り返しの採用を決める
3. builder が最終 display text と warning を返す
4. preview / PDF は builder の決定結果をそのまま描画する
...
- multi-page event で preview / PDF 共通の page count が得られること
- builder が返す row order, display text, warning が preview / PDF 両方の source of truth になること
- API route が preview と同じ theme を使ってダウンロードできること

### Component tests

- preview page が Stitch 風の主要文言と構造を持つこと
- multi-page event で全ページ preview が縦積み表示されること
...
- preview と PDF が page count / row order / warning 判定で一致する
